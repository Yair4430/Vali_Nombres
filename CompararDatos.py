...
def comparar_datos(archivo_pdf, inicio_listado, fin_listado, inicio_cert, fin_cert):
    ...
    certificados_dict = {doc: (tipo, nombre) for doc, tipo, nombre in zip(docs_cert, tipos_cert, nombres_cert)}
    ...
    for doc_c, tipo_c, nom_c in zip(docs_cert, tipos_cert, nombres_cert):
        if doc_c not in docs_listado_set:
            resultados.append((
                "-", "-", "-", "-", tipo_c, doc_c, nom_c, "0%", "0%", "No existe en listado"
            ))
